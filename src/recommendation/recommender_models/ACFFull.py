...
class ACFFull:
    def __init__(self, args, dataset):
        ...
        self.feature_shape = self.data.shape
        self.initializer_attentive = tf.glorot_normal_initializer()

        self.Gu, self.Gi, self.Pi, self.Fi = self._initialize_variables()
    ...
    def _initialize_variables(self):
        with tf.name_scope("embeddings"):
            return tf.Variable(tf.random_normal(shape=[self.num_users, self.emb_K], stddev=0.01),
                ...
                tf.Variable(self.data.emb_image, name='Fi', dtype=tf.float32)
    ...
    def _calculate_beta_alpha(self, i_p):
        ...
        f_i = tf.expand_dims(tf.nn.embedding_lookup(self.Fi, list_of_pos), axis=1)
