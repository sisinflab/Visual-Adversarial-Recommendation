...
class SaliencyMapMethodMemory(SaliencyMapMethod):
    """
    ...
    def __init__(self, model, sess=None, dtypestr='float32', **kwargs):
        ...
        """
        super(SaliencyMapMethodMemory, self).__init__(model, sess, dtypestr, **kwargs)
...
def jsma_symbolic(x, y_target, model, theta, gamma, clip_min, clip_max):
  ...
  """
    ...
    nb_features = tf.cast(tf.reduce_prod(x.shape[1:]), tf.int64)
    ...
    max_iters = tf.floor(tf.cast(nb_features, dtype=tf.float32) * gamma / 2)
    increase = bool(theta > 0)

    # tmp = np.ones((nb_features, nb_features), int)
    tmp = tf.ones(shape=(nb_features, nb_features), dtype=tf.int64)
    # np.fill_diagonal(tmp, 0)
    tmp = tf.matrix_set_diag(tmp, tf.zeros(nb_features, dtype=tf.int64))
    # zero_diagonal = tf.constant(tmp, tf_dtype)
    zero_diagonal = tf.cast(tmp, dtype=tf_dtype)
    ...
    def condition(x_in, y_in, domain_in, i_in, cond_in):
        ...
        return tf.logical_and(tf.less(tf.cast(i_in, tf_dtype), max_iters), cond_in)
    ...
    def body(x_in, y_in, domain_in, i_in, cond_in):
        ...
        grads_target = reduce_sum(grads * tf.cast(target_class, tf_dtype), axis=0)
        ...
        p1_one_hot = tf.one_hot(p1, depth=tf.cast(nb_features, tf.int32))
        # p2_one_hot = tf.one_hot(p2, depth=nb_features)
        p2_one_hot = tf.one_hot(p2, depth=tf.cast(nb_features, tf.int32))

        # Check if more modification is needed for each sample
        mod_not_done = tf.equal(reduce_sum(tf.cast(y_in, tf_dtype) * preds_onehot, axis=1), 0)
        ...
        to_mod_reshape = tf.reshape(
             to_mod, shape=(x_in.shape.as_list()))
        if increase:
        ...
        return x_out, y_in, tf.reshape(domain_out, shape=(1, domain_out.shape[1])), i_out, cond_out
