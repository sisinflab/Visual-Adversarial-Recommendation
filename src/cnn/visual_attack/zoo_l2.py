...
MAX_ITERATIONS = 20000  # number of iterations to perform gradient descent
...
@jit(nopython=True)
def coordinate_Newton(losses, indice, grad, hess, batch_size, mt_arr, vt_arr, real_modifier, up, down, lr, adam_epoch,
                      beta1, beta2, proj):
    # def sign(x):
...
@jit(nopython=True)
def coordinate_Newton_ADAM(losses, indice, grad, hess, batch_size, mt_arr, vt_arr, real_modifier, up, down, lr,
                           adam_epoch, beta1, beta2, proj):
    cur_loss = losses[0]
    for i in range(batch_size):
        grad[i] = (losses[i * 2 + 1] - losses[i * 2 + 2]) / 0.0002
        hess[i] = (losses[i * 2 + 1] - 2 * cur_loss + losses[i * 2 + 2]) / (0.0001 * 0.0001)
...
class ZOOL2:
    def __init__(self, sess, model, input_size, num_labels, batch_size=1, confidence=CONFIDENCE,
                 ...
                 use_log=False, use_tanh=False, use_resize=True, adam_beta1=0.9, adam_beta2=0.999,
        ...
        if self.use_resize:
            ...
            self.resize_op = tf.image.resize(self.resize_input, [self.resize_size_height, self.resize_size_width])
        ...
        self.newimg = self.scaled_modifier + tf.reshape(self.timg, shape=(1)+self.timg.shape)

        # prediction BEFORE-SOFTMAX of the model
        # now we have output at #batch_size different modifiers
        # the output should have shape (batch_size, num_labels)
        # print('Dimensions: ', self.timg.shape)
        self.output = model.get_logits(tf.transpose(self.newimg, perm=[0, 3, 1, 2]))
    ...
    def resize_img(self, small_x, small_y, reset_only=False):
        ...
        if reset_only:
            ...
            self.real_modifier = self.sess.run(self.resize_op, feed_dict={self.resize_size_height: self.small_x,
                                                                          self.resize_size_width: self.small_y,
    ...
    def fake_blackbox_optimizer(self):
        true_grads, losses, l2s, loss1, loss2, scores, nimgs = self.sess.run(
            [self.grad_op, self.loss, self.l2dist, self.loss1, self.loss2, self.output, self.newimg],
    ...
    def generate(self, x, y_target):
        ...
        def cw_wrap(x_val, y_val):
            return np.array(self.attack(x_val, y_val), dtype=self.np_dtype)
