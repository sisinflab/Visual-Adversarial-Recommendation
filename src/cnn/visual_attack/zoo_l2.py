...
class ZOOL2:
    def __init__(self, sess, model, height=224, width=224, batch_size=128, num_channels=3, num_labels=1000,
                 ...
                 binary_search_steps=BINARY_SEARCH_STEPS, max_iterations=1500, print_every=5,
        ...
        learning_rate: Theflag learning rate for the attack algorithm. Smaller values
        ...
        single_shape = (height, width, num_channels)
        ...
        if self.use_resize:
            ...
            # scaled up image
            self.scaled_modifier = tf.image.resize_images(self.modifier, [height, width])
            ...
            self.modifier = tf.placeholder(tf.float32, shape=(None, height, width, num_channels))
            # no resize
        ...
        self.norm_img = self.newimg + 0.5
        self.norm_img = (self.norm_img - tf.constant([0.485, 0.456, 0.406],
                                                     dtype=tf.float32)) / tf.constant([0.229, 0.224, 0.225],
                                                                                      dtype=tf.float32)
        self.norm_permuted_img = tf.transpose(self.norm_img, perm=[0, 3, 1, 2])
        self.output = model.get_logits(self.norm_permuted_img)
