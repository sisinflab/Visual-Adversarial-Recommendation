...
from cleverhans.attacks.carlini_wagner_l2 import CWL2, ZERO


class CarliniWagnerL2Std(CarliniWagnerL2):
    """
    ...
    def __init__(self, model, sess, dtypestr='float32', **kwargs):
        ...
        super(CarliniWagnerL2Std, self).__init__(model, sess, dtypestr, **kwargs)
    ...
    def generate(self, x, **kwargs):
        ...
        attack = CWL2Std(self.sess, self.model, self.batch_size, self.confidence,
                         'y_target' in kwargs, self.learning_rate,
                         self.binary_search_steps, self.max_iterations,
                         self.abort_early, self.initial_const, nb_classes,
...
class CWL2Std(CWL2):
    def __init__(self, sess, model, batch_size, confidence, targeted,
                 ...
                 abort_early, initial_const, num_labels,
                 shape):
        """
        Return a tensor that constructs adversarial examples for the given
        input. Generate uses tf.py_func in order to operate over tensors.
